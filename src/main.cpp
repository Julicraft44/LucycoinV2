...
#if defined(NDEBUG)
# error "Lucycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x413a7b48671a868209f7002d5a53cff6fd89938be0e6ef25c486c178422eb65b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Lucycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Lucycoin Signed Message:\n";
...
{
    // Lucycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 210000); // Lucycoin: 210k blocks in ~1 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Lucycoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Lucycoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Lucycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Lucycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5544277921f33f7ad710d30fd214de9e388391359c8fc4f6522f665d01641bab");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "I just turned 18 - Ukraine still in war with Russia - 22/01/23";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 24 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b798348713919bc64d7068c58f1de53210b70e3eee4eb2f8492e27947893d2e108e9591cd010bd707aa37ad339cc5728139ac4e4e633a075f0754a2e755692b4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1674244851;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084547005;

        if (fTestNet)
        {
            block.nTime    = 1674244837;
            block.nNonce   = 388512186;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x50dcd77414db4c9c7ef7f0b8ba5d406a68b7b046c52b32841afe4d977d6220ab"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc1, 0xb9, 0xd6 }; // Lucycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LucycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LucycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LucycoinMiner(CWallet *pwallet)
{
    printf("LucycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lucycoin-miner");
    ...
    try { loop {
        ...
        printf("Running LucycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LucycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LucycoinMiner, pwallet));
}
