...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Lucycoin " + FormatFullVersion();
...
unsigned int pnSeed[] =
{
    0x0100007f, 0xcff4f323, 0x5b60ed28, 0x2d4d9937, 0x6c3d95b2, 0x951c33d5, 0x2d4d4def
};
...
void ThreadOpenConnections()
{
    ...
    loop
    {
        ...
        if (addrman.size()==0 && (GetTime() - nStart > 20) && !fTestNet)
        {
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Lucycoin is probably already running."), addrBind.ToString().c_str());
        else
